...
| `GUI_CONFIRMATION_REQUIRED` | Transcription was requested without checking the explicit confirmation. | Expensive GPU work and output publication do not begin accidentally. | Review dry-run, check the confirmation, and queue deliberately. |
...
| `GUI_CORRECTION_DICTIONARY_INVALID` | Only one of dictionary path and project ID was supplied, or the dictionary was invalid. | A dictionary cannot lose project scope or provenance. | Supply both matching values or neither. |
| `GUI_CORRECTION_CREDENTIAL_MISSING` | The configured OpenRouter key environment variable is absent. | Credentials are never requested from or persisted by the browser. | Set the key in the server terminal and restart the GUI. |
