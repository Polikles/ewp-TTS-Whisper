...
| `GUI_CORRECTION_CREDENTIAL_REJECTED` | The provider answered, but rejected the supplied key with HTTP 401 or 403. | Authentication failure is distinguished from network failure and missing models. | Check that the key is current, correctly pasted, and authorized for the provider. |
