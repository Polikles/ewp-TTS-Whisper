...
| `GUI_REVIEW_SPLIT_BOUNDARY_REQUIRED` | A split was requested where the text cursor did not separate two nonempty word or sentence parts. | The current block remains unchanged. | Place the cursor at a whitespace boundary between the intended parts, then select **Split at cursor**. |
